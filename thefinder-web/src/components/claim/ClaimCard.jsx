...
export default function ClaimCard({ claim, post, mode, busy, onReview, onReject, onConfirm, onCancel }) {
  ...
  return <article className="rounded-[34px] border border-[#237596] bg-white p-5 sm:p-7">
    ...
    {post && <section className="mt-8 grid gap-5 rounded-[30px] border border-[#237596] p-3 sm:grid-cols-[42%_1fr]" aria-label={`Bài đăng ${post.title}`}>
      ...
      <div className="flex min-w-0 flex-col p-2"><div className="flex items-center gap-3"><span className="rounded-full bg-[#287f9f] px-3 py-1 text-sm text-white">{post.type === 'LOST' ? 'Tìm đồ' : 'Trả đồ'}</span><h3 className="truncate text-2xl font-semibold">{post.title}</h3></div><div className="mt-4 rounded-3xl bg-[#d9d9d9] p-4"><p className="text-[#237596]">#{post.categoryName || 'Khác'}</p><p className="mt-2 line-clamp-4">{post.description || 'Không có nội dung chi tiết.'}</p></div><p className="mt-auto pt-4">Tác giả: <Link to={`/users/${post.authorId}`} className="font-medium hover:text-[#237596] hover:underline">{post.authorName}</Link></p><p>Trạng thái: <span className="text-[#237596]">{post.status}</span></p></div>
    </section>}
    {mode === 'received' && isActive && <div className="mt-7 flex flex-wrap gap-5"><button disabled={busy} type="button" onClick={() => onReject(claim)} className="h-12 flex-1 rounded-full border border-[#237596] disabled:opacity-50">Bác bỏ đơn</button>{claim.status === 'SUBMITTED' ? <button disabled={busy} type="button" onClick={() => onReview(claim)} className="h-12 flex-1 rounded-full bg-[#287f9f] text-white disabled:bg-[#d9d9d9]">Tôi sẽ kiểm tra</button> : <button disabled={busy} type="button" onClick={() => onConfirm(claim)} className="h-12 flex-1 rounded-full bg-[#fb5353] text-white disabled:opacity-50">{post?.type === 'LOST' ? 'Tôi đã nhận được đồ' : 'Tôi đã trả lại đồ'}</button>}</div>}
